"""
Extract sequences from protein alignments and annotate coupling data.

This script processes coupling files, maps them to protein alignments, and extracts
sequence information for PFAM domain pairs.
"""
import os
...
@click.option('--aln_widths', default='./pfam_align_lengths.csv',
              help='Path to PFAM alignment lengths CSV')
@click.argument('coupling_files', nargs=-1)
def scan_args(overwrite, aln_widths, coupling_files):
    """
    Process coupling files and extract sequence information.

    Args:
        overwrite: Whether to overwrite existing files
        aln_widths: Path to PFAM alignment lengths CSV
        coupling_files: List of coupling files to process
    """
    # Widths of the PFAM alignments - used to locate boundary between PFAMs in glue file
    aln_widths = pd.read_csv(aln_widths, header=None, names=['pfam_id', 'pfam_len'])

    # Existing set of alignments
    aln = pd.read_csv('aln.csv')
    ...
    for fn in coupling_files:
        ...
        pre_seq = pd.concat([
            couplings[['lft_pos', 'lft_aa']].rename(columns={'lft_pos': 'pos', 'lft_aa': 'aa'}),
            couplings[['rgt_pos', 'rgt_aa']].rename(columns={'rgt_pos': 'pos', 'rgt_aa': 'aa'})
        ]).drop_duplicates().sort_values('pos')

        # Select the two alignments of interest (Mycoplasma genitalium proteins)
        selected = aln[
            (aln['lft_pfam_id'] == pfam_id_lft) & (aln['rgt_pfam_id'] == pfam_id_rgt)
        ].sort_values('order')
        ...
        if selected.shape[0] == 2:
            ...
            for ridx, row in selected.iterrows():  # Each row is one alignment
                # Changed to list comprehension with filter - old code had wrong positions
                # Skip over lowercase letters in the sequence
            ...
            # If throwing exception here, number of rows not matching - align.csv entry not correct
            pre_seq['sp_id'] = sp_id
            pre_seq['sp_aa'] = sp_aa
            pre_seq['sp_pos'] = sp_pos
        else:
            print(f"[{pfam_id_lft}-{pfam_id_rgt}] shape is {selected.shape[0]}\n")
            for cl in ('sp_id', 'sp_aa', 'sp_pos'):
                pre_seq[cl] = None

        pre_seq['pfam_id'] = pre_seq.apply(
            lambda row: pfam_id_lft if row['pos'] <= pfam_lft_len else pfam_id_rgt,
            axis=1
        )
        ...
        # Merge normalized data with sequence position information
        norm2 = pd.merge(
            left=norm,
            right=pre_seq[['pos', 'sp_id', 'sp_pos']].rename(
                columns={'pos': 'lft_pos', 'sp_id': 'lft_sp_id', 'sp_pos': 'lft_sp_pos'}
            ),
            on='lft_pos',
            how='left'
        )
        norm3 = pd.merge(
            left=norm2,
            right=pre_seq[['pos', 'sp_id', 'sp_pos']].rename(
                columns={'pos': 'rgt_pos', 'sp_id': 'rgt_sp_id', 'sp_pos': 'rgt_sp_pos'}
            ),
            on='rgt_pos',
            how='left'
        )
        ...
        # If missing alignment rows, they must be computed - print out sequences
        if selected.shape[0] != 2:
